...
// Update Order Track
export const updateOrderTrack = (id, order) => async (dispatch) => {
  try {
    dispatch({ type: UPDATE_ORDER_REQUEST })
    ...
    const { data } = await api.put(
      `/api/v1/admin/order/track/${id}`,
      order,
      config
    )

    dispatch({ type: UPDATE_ORDER_TRACK_SUCCESS, payload: data.success })
  } catch (error) {
    dispatch({
      type: UPDATE_ORDER_FAIL,
      payload: error.response.data.message,
