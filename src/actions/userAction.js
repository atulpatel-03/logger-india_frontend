...
export const sendMessage = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await api.post(`/api/v1/sendmessage`, userData, config)

    dispatch({ type: USER_MSG_SUCCESS, payload: data.message })
  } catch (error) {
    dispatch({
      type: USER_MSG_FAILED,
      payload: error.response.data.message,
    })
  }
}
...
//Google Login
export const googleLogin = (sendData) => async (dispatch) => {
  try {
    dispatch({ type: LOGIN_REQUEST })
    console.log("loginnnn with google")
    const config = { headers: { "Content-Type": "application/json" } }

    const { data } = await api.post(`/api/v1/googlelogin`, sendData, config)
