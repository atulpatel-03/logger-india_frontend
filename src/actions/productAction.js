...
export const getProduct =
  ...
  async (dispatch) => {
    try {
      ...
      if (subcategory) {
        link = `/api/v1/products?page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&subcategory=${subcategory}&ratings[gte]=${ratings}`
      }
      if (brand) {
        console.log("sahi h", brand)
        var brandSize = brand.length
        console.log("brand size", brandSize)
        for (var i = 0; i < brandSize; i++) {
          console.log("brand", i)
          link = link + `&brand=${brand[i]}`
        }
